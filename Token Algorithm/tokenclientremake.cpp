...
void tokenring(int nextSoc, int prevSoc, int philosID){
    ...
    while (!done){
        ...
        if (token == true){//I have a token
            if (wantEat && !(eaters[(philosID + 4) % 5] || eaters[(philosID + 1) % 5])){//do we want to eat and can we eat?
                eatAllowed = true;
                eaters[philosID] = true;
            }
            else if (!wantEat && eaters[philosID])//do we want to keep eating
                eaters[philosID] = false;
            ...
            //cout<<"Phil "<<philosID<<": Just sent: "<<converter<<endl;
            ...
            ss<<converter;
            token = true;
            //cout<<"Phil "<<philosID<<": Just got: "<<converter<<endl;
...
int main(int argc, char **argv){
    ...
    cout<<"Phil "<<philosID<<": "<<"I have finished eating and I'm full. Shutting down now...\n";
}